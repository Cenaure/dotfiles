...
Item {
  ...
  function switchRelative(delta) {
    const step = BarConfig.scrollExistingOnly ? (delta > 0 ? "e+1" : "e-1") : (delta > 0 ? "+1" : "-1");
  ...
  Row {
    ...
    Repeater {
      ...
      delegate: Item {
        ...
        readonly property color contentColor: slot.isActive ? BarConfig.workspaceActiveForeground : slot.isUrgent ? BarConfig.workspaceUrgentColor : slot.isOccupied ? BarConfig.workspaceColor : BarConfig.workspaceEmptyColor

        height: BarConfig.workspaceHeight
        width: Math.max(BarConfig.workspaceWidth, content.implicitWidth + BarConfig.workspacePadding * 2)
